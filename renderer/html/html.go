...
func (self *withWriter) SetHTMLOption(config *Config) { config.Writer = self.value }
...
// TODO: Was type struct but clearly this should be type bool; now need simplify
// the code based on that fact.
type withHardWraps bool
...
func (self *withXHTML) SetConfig(config *renderer.Config) {
	config.Options[optXHTML] = true
}

// TODO: I hate these struct names
func (self *withXHTML) SetHTMLOption(c *Config) {
	c.XHTML = true
}
